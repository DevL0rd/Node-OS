...
function pm.getSelectedProcessID()
  ...
  function pm.createProcess(path, settings)
    ...
    if settings.isService == nil then
    ...
    if not settings.x then
      settings.x = math.ceil(termWidth / 2 - (settings.width / 2))
    end
    if not settings.y then
      if (settings.height % 2) == 0 then
        settings.y = math.ceil(termHeight / 2 - (settings.height / 2))
      else
        settings.y = math.ceil(termHeight / 2 - (settings.height / 2)) + 1
      end
    end
